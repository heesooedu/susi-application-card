...
- `APPLICATIONS`: 지원 카드 한 장당 한 행(long format). 내부 ID 열은 숨기고 학생 이름·반·번호와 교사 코멘트를 표시하며 이름순 정렬과 교차색상을 적용. 카드별로 최근 3개 학년의 학년도·50% 컷·70% 컷·기타 메모·입시결과 URL을 선택 저장
...
이전 스키마의 `APPLICATIONS` 시트는 새 코드가 처음 실행될 때 학생 표시 열, 교사 코멘트 열, 3개년 입시결과 열을 자동 삽입합니다. 기존 카드의 이름·반·번호는 `student_id`를 기준으로 역채우며 원래 카드 값은 보존합니다.

## 교사 코멘트

교사는 `APPLICATIONS` 시트의 `teacher_comment` 열에 카드별 코멘트를 직접 입력합니다. 코멘트가 있으면 학생 웹 화면 상단에 알림이 나타나고 해당 카드에 `코멘트 있음` 배지가 표시됩니다. 학생은 카드의 `전체 내용 펼쳐보기`에서 코멘트를 확인할 수 있으며 학생 저장 요청으로는 이 값을 변경할 수 없습니다.
