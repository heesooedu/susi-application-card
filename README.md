...
- `APPLICATIONS`: 지원 카드 한 장당 한 행(long format). 한국어 표시 헤더를 사용하고 내부 ID 열은 숨깁니다. 학생 이름순으로 정렬하며 같은 학생의 여러 카드는 동일한 배경색, 다음 학생 그룹은 교차 배경색으로 표시합니다.
...
이전 스키마의 `APPLICATIONS` 시트는 새 코드가 처음 실행될 때 필요한 열을 자동 삽입하고 표시 헤더를 한국어로 변환합니다. 기존 카드의 이름·반·번호는 `student_id`를 기준으로 역채우며 원래 카드 값은 보존합니다.

## 교사 코멘트

교사는 `APPLICATIONS` 시트의 `교사 코멘트` 열에 카드별 코멘트를 직접 입력합니다. 새 코멘트가 있으면 학생 웹 화면 상단에 알림과 카드별 `새 코멘트` 배지가 표시됩니다. 학생이 `확인했어요`를 누르면 알림은 사라지지만 코멘트 본문은 계속 볼 수 있습니다. 교사가 내용을 변경하면 fingerprint가 달라져 다시 새 코멘트로 표시됩니다. 학생 저장 요청으로는 코멘트를 변경할 수 없습니다.
